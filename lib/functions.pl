...
sub wakeup {
		my $status = decode_utf8("こんにちは！さいたんbotがログインしたよ！");
		&talk($status, undef);
}
...
sub check_category {
		...
		if ($is_mentioned == 1) {
				foreach my $replyonly (@{$conf->{replyonly}}) {
						foreach my $keyword (@{$conf->{$replyonly}->{keywords}}) {
								if ($text =~ decode_utf8($keyword)) {
										return $replyonly;
										last;
								}
...
sub fav {
		my $fav_id = shift;
		eval {
				$actions->create_favorite($fav_id);
		};
		if ($@) {
				if ($@ =~ /favorite per day/) {
						&talk(decode_utf8("ふぁぼ規制ﾅｰｰｰｰｰｰｰｰｰｰｰｰｰｰ"), undef);
						warn "$@\n";
				} else {
						&talk(decode_utf8("ふぁぼ失敗なう＞＜ 当該ID: $fav_id"), undef);
						warn "$@\n";
				}
		} else {
...
sub limit {
		...
		my $cv = AE::cv;
		my $timer;
		$timer = AE::timer(180, 180, sub {
						my $status = &talk(decode_utf8("規制解除てす$hashtag"), undef);
						if ($status == 1) {
								undef $timer;
								$cv->send;
						}
